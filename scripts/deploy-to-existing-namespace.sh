...
NAMESPACE="icn-system"  # Use the existing namespace
TMP_DIR="/tmp/icn-k8s-deploy"
...
echo "Using the existing namespace: ${NAMESPACE}"
...
# 5. Update all Kubernetes YAML files to use the existing namespace
echo "[5/7] Updating Kubernetes YAML files..."
for file in kubernetes/*.yaml; do
  sed -i "s|\${NAMESPACE}|${NAMESPACE}|g" "$file"
done

# Also update any image pull policies to Never since we're loading the image locally
find kubernetes/ -name "*deployment.yaml" -exec sed -i 's/imagePullPolicy: IfNotPresent/imagePullPolicy: Never/g' {} \;
...
ssh -i ${SSH_KEY} matt@${REMOTE_HOST} "
  ...
  # Setup kubeconfig - just try to use the existing setup
  echo 'Using existing Kubernetes configuration'
  
  # Check if we can access the namespace
  if ! kubectl get namespace ${NAMESPACE} &>/dev/null; then
    echo 'Error: Cannot access the ${NAMESPACE} namespace'
    echo 'You may need to run this script with sudo or as a user with Kubernetes access'
    exit 1
  fi
  ...
  kubectl apply -f ${TMP_DIR}/persistent-volume-claims.yaml || echo 'Warning: Some PVCs may already exist'
...
echo "================================================================"
echo "Deployment completed to namespace: ${NAMESPACE}"
echo ""
echo "To check status, try running:"
echo "./scripts/check-icn-system.sh"
echo "================================================================" 