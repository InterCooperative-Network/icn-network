#!/bin/bash
# Bootstrap IPv6-focused ICN Network
# This script initializes a new ICN network from scratch with IPv6 configuration.

set -e

# Configuration
NETWORK_NAME=${1:-"icn-testnet-ipv6"}
NODE_COUNT=${2:-3}
BASE_PORT=9000
DATA_DIR="./data/icn"
CONFIG_DIR="./config"
...
# Print header
echo -e "${BLUE}======================================${NC}"
echo -e "${BLUE}ICN IPv6 Network Bootstrap Script${NC}"
echo -e "${BLUE}======================================${NC}"
...
if ! ip -6 addr show | grep -q "scope global"; then
    echo -e "${YELLOW}Warning: No global IPv6 address found. Using local addresses for testing.${NC}"
    USE_LOCAL_IPV6=true
else
...
cp ${CONFIG_DIR}/network-ipv6.yaml ${CONFIG_DIR}/${NETWORK_NAME}.yaml
...
for i in $(seq 1 $NODE_COUNT); do
    ...
    BOOTSTRAP_PEERS="${BOOTSTRAP_PEERS}\"/ip6/::1/tcp/${NODE_PORT}/p2p/\${${NODE_ID}_PEER_ID}\""
done
BOOTSTRAP_PEERS="${BOOTSTRAP_PEERS}]"
...
node_id: "${NODE_ID}"
node_type: "full"
listen_addr: "/ip6/::1/tcp/${NODE_PORT}"
peer_id: "${PEER_ID}"
...
cat > ${CONFIG_DIR}/genesis-ipv6.json <<EOF
{
  "network_name": "${NETWORK_NAME}",
  "creation_time": "$(date -u +"%Y-%m-%dT%H:%M:%SZ")",
  "federation": {
    "id": "ipv6-genesis-federation",
    "name": "IPv6 Genesis Federation",
    "founding_members": [
EOF
...
for i in $(seq 1 $NODE_COUNT); do
    ...
    if [ $i -gt 1 ]; then
        echo "      }," >> ${CONFIG_DIR}/genesis-ipv6.json
    fi
...
echo -e "${GREEN}Genesis configuration created at ${CONFIG_DIR}/genesis-ipv6.json${NC}"

# Create IPv6-specific launcher script - modified to use xterm or another terminal
echo -e "${YELLOW}Creating launcher script...${NC}"
cat > ./start_ipv6_network.sh <<EOF
#!/bin/bash
...
echo -e "${BLUE}======================================${NC}"
echo -e "${GREEN}IPv6 Network bootstrap complete!${NC}"
echo -e "${GREEN}To start the network, run: bash ./start_ipv6_network.sh${NC}"
echo -e "${BLUE}======================================${NC}" 