...
# Print header
echo "================================================================"
echo "ICN Network Transfer and Deployment Script"
echo "================================================================"
...
# Step 1: Update all YAML files to use the correct namespace
echo "[1/3] Updating Kubernetes YAML files to use namespace: ${NAMESPACE}"
...
# Update all YAML files to use the specific namespace
for file in kubernetes/*.yaml; do
  # Skip the namespace.yaml file as we won't need it (using existing namespace)
  if [ $(basename "$file") == "namespace.yaml" ]; then
    echo "Skipping namespace.yaml as we're using an existing namespace"
    continue
  fi
  
  # Replace any timestamp-based namespace with our target namespace
  sed -i -E "s/namespace: icn-network-[0-9]+/namespace: ${NAMESPACE}/g" "$file"
  
  # Also replace any ${NAMESPACE} placeholders
...
echo "To check status later, connect to the remote server and run:"
